...
pairwiseContinuityAnalysis_v2 <- function(object, subsample.genes=F,
  ...
  pair.res <- foreach(i = 1:nrow(ids.iterator), .combine="c") %dopar% {
    ...
    ident.res <- scoreCellsByIdentitySignature(object=pairObject, id1 = id1, id2 = id2, 
                                               num.sig.genes = num.sig.genes,genes.use=genes.use, dir=dir)
    ...
    list(c(id1, id2), dstats, cellid.params, ident.signature)
  ...
  pair.res <- list(pairwise.statistics=id.pairs, signature.scores=signature.scores, cell.names=object@cell.names)
...
scoreCellsByIdentitySignature <- function(object, id1, id2, num.sig.genes=30, method = 'pct_or_diff', 
  ...
  genes.fc <- sortGenes.logFC(object, id1, id2)
  genes.pct <- sortGenes.logFC(object, id1, id2)
  genes.fc.1 <- names(genes.fc)[genes.fc>0]
  genes.fc.2 <- names(genes.fc)[genes.fc<0]
  genes.pct.1 <- names(genes.pct)[genes.pct>0]
  genes.pct.2 <-  names(genes.pct)[genes.pct<0]
  ...
  if(min(length(genes.1), length(genes.2)) < 5){
    ...
    return(list(sig.scores, c(genes.1, genes.2)))
  }
...
get.markers.by.diff <- function(de.results, id, n.markers,  ident=1){
  require(dplyr)
  require(tibble)
  if(ident==1){
    return(de.results %>% 
             tibble::rownames_to_column(var='gene') %>%
             filter(p_val < 1e-3, avg_logFC > .2) %>% 
             top_n(n.markers, avg_logFC) %>%
             pull(gene))
  }
  else if(ident==2){
    return(de.results %>% 
...
sortGenes.logFC <- function(object, ident.1, ident.2, genes.use=NULL, thresh=0.2){
	if(is.null(genes.use)) genes.use = rownames(object@data)
	else genes.use = genes.use[genes.use %in% object@data]

	data.1 = t(as.matrix(object@data[, object@ident==ident.1]))
	data.2 = t(as.matrix(object@data[, object@ident==ident.2]))
	mean.1 = apply(data.1, 2, mean)
	mean.2 = apply(data.2, 2, mean)
	
	diff <- mean.1 - mean.2
	diff <- diff[abs(diff) > thresh]
	return(diff[order(diff)])
